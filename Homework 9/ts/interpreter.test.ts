...
const captureLogs = (fn: () => void): unknown[][] => {
  const logs: unknown[][] = [];
...
function expectExprToBe(src: string, expected: RuntimeValue) {
  const ast = parseExpression(src);
  expect(interpExpression({}, ast)).toEqual(expected);
}

function expectStmtToTransform(src: string, before: State, after: State) {
...
function expectStateToBe(program: string, expected: State) {
...
describe("interpExpression", () => {
  it("handles literals and variable lookup", () => {
    ...
    const child: State = { [PARENT_STATE_KEY]: parent, y: 2 };
...
describe("interpStatement", () => {
  it("handles declarations, assignments, and scope resolution", () => {
    ...
    const parent: State = { x: 1 };
    const child: State = { [PARENT_STATE_KEY]: parent };
