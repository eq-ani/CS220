...
export function interpExpression(s, e) {
    switch (e.kind) {
        case "number":
        case "boolean":
            return e.value;
        ...
        case "operator": {
            ...
            if (op === "&&") {
                ...
                if (typeof r !== "boolean")
                    throw new Error("Right operand of && must be boolean");
                return r;
            }
            if (op === "||") {
                const l = interpExpression(s, e.left);
...
export function interpStatement(s, stmt) {
    switch (stmt.kind) {
        case "let": {
        ...
        case "assignment": {
            let c = s;
            while (c && !Object.prototype.hasOwnProperty.call(c, stmt.name))
                c = c[PARENT_STATE_KEY];
            if (!c)
        ...
        case "if": {
            ...
            const scope = { [PARENT_STATE_KEY]: s };
...
export function interpProgram(p) {
    const g = {};
    for (const st of p)
        interpStatement(g, st);
    return g;
}
