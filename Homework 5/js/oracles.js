...
export function stableMatchingOracle(makeStableMatching) {
    for (let i = 0; i < NUM_TESTS; ++i) {
        ...
        const matchesCa = new Array(N).fill(-1);
        const matchesCo = new Array(N).fill(-1);
        ...
        const rankCo = new Array(N);
        for (let co = 0; co < N; co++) {
            rankCo[co] = new Array(N);
            for (let r = 0; r < N; r++) {
                const cand = companies[co][r];
                rankCo[co][cand] = r;
            }
        }
        const rankCa = new Array(N);
        for (let ca = 0; ca < N; ca++) {
            rankCa[ca] = new Array(N);
            for (let r = 0; r < N; r++) {
...
export function stableMatchingRunOracle(makeStableMatchingTrace) {
    for (let i = 0; i < NUM_TESTS; i++) {
        ...
        const nextCo = new Array(N).fill(0);
        const nextCa = new Array(N).fill(0);
        ...
        const outCo = new Array(N).fill(-1);
        const outCa = new Array(N).fill(-1);
