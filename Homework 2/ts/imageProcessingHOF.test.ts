...
describe("imageMapCoord", () => {
  ...
  it("should not modify the original image", () => {
    function darken(img: Image, x: number, y: number): Color {
  ...
  it("should apply a function that uses the left neighbor's color", () => {
    const copyLeft = (img: Image, x: number, y: number): Color => (x > 0 ? img.getPixel(x - 1, y) : img.getPixel(x, y));
...
describe("imageMapIf", () => {
  ...
  function darken(c: Color): Color {
    return [c[0] - 2, c[1] - 2, c[2] - 2];
  }
  it("should return a different image", () => {
    function tru(): boolean {
      return true;
    }
    const input = Image.create(10, 10, COLORS.WHITE);
...
describe("mapWindow", () => {
  ...
  it("should return a different image", () => {
    ...
    const intervalx = [0, 0];
    const intervaly = [0, 0];
    const output = mapWindow(input, intervalx, intervaly, (p: Color) => p);
    assert(input !== output);
  });
  it("should work for an image that is not square", () => {
    ...
    const intervalx = [0, 6];
    const intervaly = [2, 7];
    const output = mapWindow(input, intervalx, intervaly, (p: Color) => [p[0] - 2, p[1] - 2, p[2] - 2]);
